...
const removeProfilePicture = () => {
    pictureForm.post(route('profile.avatar.remove'), {
        onFinish: () => {
            pictureForm.avatar = null;
            visible.value = false;
        },
...
<template>
    <span class="pi pi-camera p-2 rounded-md m-1 border border-transparent bg-neutral-500 hover:border-neutral-400 transition" @click="visible = true"></span>

    <Dialog v-model:visible="visible" modal header="Update your avatar" :style="{ width: '25rem' }">
        <template #header>
            <div class="inline-flex items-center justify-center gap-2 text-white bg-se">
                <span class="font-bold whitespace-nowrap text-white">Update Your Avatar</span>
            </div>
        </template>
        <form @submit.prevent>
            <div>
                    ...
                    <template #header="{ chooseCallback, files }">
                        <div class="flex items-center w-full gap-x-4" v-show="files.length == 0">
                            ...
                            <div class="h-10 rounded-lg bg-secondary flex-grow flex items-center px-4 text-neutral-500 select-none">
                                {{ hasError ? 'Please select an image' : 'Choose profile picture'}}
                            </div>
                        </div>
                    </template>
                    <template #content="{ files, removeFileCallback }">
                        <div class="flex flex-col">
                            <div v-if="files.length > 0">
                                <div class="relative">
                                    <img role="presentation" :alt="files[0].name" :src="files[0].objectURL" class="w-full aspect-square object-cover rounded-md"/>
                                    <button
                ...
                <div class="flex justify-center my-4">
                    <Button label="Remove Profile picture" icon="times" @click="removeProfilePicture"/>
                </div>
            </div>
        </form>
        <template #footer>
            <div class="flex items-center gap-x-4 justify-end w-full">
                ...
                <Button label="Change" @click="changeProfilePicture"/>
            </div>
        </template>
    </Dialog>
</template>
